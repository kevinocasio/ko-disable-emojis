<?php
/**
 * Plugin Name: KO Disable Emojis
 * Plugin URI:  https://kevinocasio.com/tools/ko-disable-emojis/
...
define('KO_DE_VERSION', '1.0.0');
define('KO_DE_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('KO_DE_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ko_de_brand_url($path = '/tools/') {
 ...
 */
function ko_de_activate()
{
	if (get_option('ko_de_enabled') === false) {
		update_option('ko_de_enabled', '1');
	}
}
register_activation_hook(__FILE__, 'ko_de_activate');

/**
 * 2. Core Logic: Dequeue WordPress Emoji Scripts and Styles
 */
function ko_de_apply_optimization()
{
	if ((int) get_option('ko_de_enabled', '1') !== 1) {
		return;
	}
	...
	add_filter('tiny_mce_plugins', 'ko_de_remove_tinymce_emoji');

	// Remove DNS prefetch for s.w.org
	add_filter('emoji_svg_url', '__return_false');
}
add_action('init', 'ko_de_apply_optimization');

function ko_de_remove_tinymce_emoji($plugins)
{
	if (is_array($plugins)) {
...
add_action('admin_menu', 'ko_de_admin_menu');

/**
 * 4. AJAX Handler for Instant Dashboard Toggle
 */
function ko_de_ajax_toggle_status()
{
	check_ajax_referer('ko_de_toggle_nonce', 'nonce');
	...
	update_option('ko_de_enabled', $enabled);
...
add_action('wp_ajax_ko_de_toggle_status', 'ko_de_ajax_toggle_status');

/**
 * 5. Action Link on Plugins Screen
 */
function ko_de_settings_action_link($links)
{
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ko_de_settings_action_link');

/**
 * 6. Conditional Admin Asset Enqueuing (Strict Anti-Bloat)
 */
function ko_de_enqueue_admin_assets($hook)
{
	if ('toplevel_page_ko-plugins-main' !== $hook) {
		return;
	}
	$ver = file_exists(KO_DE_PLUGIN_DIR . 'assets/admin.css') ? filemtime(KO_DE_PLUGIN_DIR . 'assets/admin.css') : KO_DE_VERSION;
	wp_enqueue_style('ko-de-admin-css', plugins_url('assets/admin.css', __FILE__), array(), $ver);
}
add_action('admin_enqueue_scripts', 'ko_de_enqueue_admin_assets');
...
if (!function_exists('ko_plugins_suite_dashboard_html')) {
	function ko_plugins_suite_dashboard_html()
	{
		...
		$hub_url = ko_de_brand_url('/tools/');
		$author_url = ko_de_brand_url('/');
		...
		<div class="wrap ko-dash-wrap">
			...
			<div class="ko-dash-grid">
				<?php foreach ($suite_plugins as $plugin):
					...
					$plugin_url = ko_de_brand_url('/tools/' . $plugin['slug'] . '/');
					?>
